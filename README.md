...
## 参考与来源：CoreSwap 的思路复现

本项目的起点不是「把 Minecraft 搬到 C++」这个念头本身，而是
[**unknowbug/CoreSwap**](https://github.com/unknowbug/CoreSwap) 提出的思路：
用 JDK 的 FFM 把服务端热点子系统的内层循环交给原生代码。

Cava 对这条思路做的是**理论上的复现 + 实际上的测试**（不是代码移植，未复用 CoreSwap 的任何代码）：

- **理论复现**：把「什么样的逻辑值得搬」形式化成四条可检验的条件（见下文「可复用判据」），
  推演出**什么时候搬得快、什么时候搬了更慢** —— 决定因素是「那块活儿占 tick 多少、数据好不好交、能不能一次调用干一整批」，
  而不是「Java 还是 C++」。
- **实际测试**：在真实整合包上把生物寻路 / 实体开销 / 红石三个子系统分别做到可运行、可**逐 tick 比对**的程度，
  再用 native on/off 两条腿的成对实测来裁决（方法与数字见下文结论表与 docs/CAVA-gates.md）。
- **得出的结论**：思路成立，但**目标选错时语言救不回来** —— 本整合包上三个子系统**全部不接管**
  （两个实测净亏、一个让位），默认配置与不装 Cava 行为一致。

完整的推导过程、实测数字与工程教训写在博客里：
[**《我把 Minecraft 的寻路搬到了 C++，结果它更慢了》**](https://blog.imsng.top/post/%E6%88%91%E6%8A%8A-minecraft-%E7%9A%84%E5%AF%BB%E8%B7%AF%E6%90%AC%E5%88%B0%E4%BA%86-c%E7%BB%93%E6%9E%9C%E5%AE%83%E6%9B%B4%E6%85%A2%E4%BA%86)

> 结论是负面的，但负面结论是可复用的：**知道不该做什么，和知道该做什么一样值钱。**
