...
| **P1-CROSS** | 用户裁定 R7：赌"跨 tick 复用" | 🔄 进行中 | ①先量**复用命中率**（低 ⇒ 这一注当场输）；②跨 tick 失效正确性（推进真实 tick 后必须看到新地形，关掉钩子必红）；③按 P1-NET 配对方法测净收益（与 −14.75 µs/tick 可比）。默认值仍 false，改默认由 captain 拍板 |
