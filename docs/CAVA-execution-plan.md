...
| **P1-CROSS** | 用户裁定 R7：赌"跨 tick 复用" | ✅ 完成（`c527494`/`1d37037`）→ **判负（两重输）** | ①**命中率 1/1972 = 0.05%**（`mirrorPushes == nativeCalls`，连同 tick 复用都是 0：真实负载里没有"第二个相同矩形"）；②**结构输**：把推送成本整块抹零（不可达上界）四条腿仍 ≤ 0（−14.63/−7.02/−7.09/−2.51）⇒ 符号由**回退白付**（8.5–16.7 µs/tick）决定，推送只有 8.4–11.0。正确性那一半做成了：`xtick` 推进真实 tick（2/6/21/101）全 PASS，关钩子四条全 RED（`stale-result(B==A)`、129 节点陈旧路径 vs Java 128）。默认值未动。**唯一剩下的路 = R1**（sound 窗口信号，ABI 决策，量级 +7~+15 µs/tick；天花板 = 寻路总耗时 34–45 µs/tick ≈ tick 的 7–9%） |
