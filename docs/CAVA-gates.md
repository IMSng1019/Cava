...
- **R6 机械对拍**：比对脚本的字段名与回执字段名要有一条**测试**去对拍（本轮已经因为笔误漏掉一个字段），
  P1-NET 已落地（`ReceiptFieldParityTest`，并顺带查出 `site:` 复合记号以前一个都没比、`(缺)==(缺)` 静默通过）。
- **R7（用户裁定，2026-09-24）**：P1-NET 判定 NO-GO 之后，用户选择**先赌"跨 tick 复用"这一注**，而不是立刻收口。
  实验设计（P1-CROSS 流）：**先量命中率**（跨 tick 复用要求"同一个矩形"再次被求解，而真实负载里每只怪/每个目标
  的矩形都不同 ⇒ 命中率低则这一注当场就输，不必再测性能）→ **再证跨 tick 失效正确性**（改窗口内方块后
  **推进真实 tick** 再求解必须看到新地形；关掉失效钩子必须变红）→ **再按 P1-NET 同一套配对方法测净收益**，
  与 `−14.75 µs/tick` 直接可比。默认值仍为 false，**改默认值由 captain 拍板**。
  若这一注也输，剩下的路是"整块世界常驻镜像 + 增量更新"（量级明显更大，届时再评估）。


**P1-NET 结果（提交 `09ea1dc`，台账 `docs/CAVA-p1-net-notes.md`）—— R2 三条判据逐条不满足 ⇒ NO-GO**

| 判据 | 结果 |
| --- | --- |
| ① 真实 AI 负载净收益为正 | **不满足**：决定性配对腿 **−14.75 / −14.71 µs/tick**（两次差 0.2%），7/7 条腿符号一致全为负 |
| ② 回退率 ≤5% | **不满足**：真实负载 21.1 / 31.8 / 34.6 / 37.5% |
| ③ 短程分流后再测 | **不满足**：分流已实现并实测（`-Dcava.pathfind.gate.minBlocks`，O(1)，默认 0），但它**只能止亏不能转正**；数据支持的最优阈值 = 不接管（T≥12 格时 gated=490/490、takeover=0，等于把开关关掉） |

**亏损的机制（同一腿内可分解，加得上）**：①**接管成功的调用本身就更慢**：原生 157.7 µs/次 vs Java 129.6 µs/次
（1.22x，跨界固定成本）⇒ ≈10.2 ms/2400tick；②**回退 = 白付一次原生**（157.7 µs/次）⇒ ≈25.2 ms/2400tick；
两项合计 35.4 ms，与 `pairNetMs` 精确对上。

**为什么合成场景的 2x 在真实负载上不存在（这是本轮最重要的一个数）**：
真实 AI 每次调用的**距离 p50=7 / p90=10 / p99=10 格**、返回节点数 p50=7~8 / max≈26，而原版节点预算恒为 **560**
⇒ 真实负载**根本没有大搜索空间**（合成场景是 128–503 节点）。而且"短程=便宜"在这份负载上不成立：
5–8 格桶（239.9 µs/次）比 ≤2 格桶（99.9 µs/次）**更贵**（长尾来自"目标走不到 ⇒ 预算跑满"）。

**可证伪对照**：把分流阈值改坏（=0）→ 净收益从 0 掉到 **−27.4 µs/tick**；`-GateMin 99999` → `native=0`；
`-Compare off-F vs gatebad` 与 `off-F vs on-F` 都是 **CONSISTENT EXIT=0**（分流不改变结果）。
副产物：`pairSame=66%` ⇒ 两条实现在真实负载上**大面积逐节点一致**（正确性没有退化）。

**R6 机械对拍落地**（`ReceiptFieldParityTest`）：脚本字段表 ⊆ 回执键名、回执每个键必须"被比 or 在 EXEMPT 里带理由"
（**多一个也要红**）、真实回执样本里每个要比字段都取得到值；并**顺手查出更多洞**：PERFDETAIL 要比字段 8 → 15、
`site:` 的复合记号**以前一个都没比**、脚本里 `(缺)==(缺)` 的静默通过改成**直接红**。

> **结论：在 R1（窗口策略）不变、ABI 冻结的前提下，P1 的原生接管在这套整合包的真实 AI 负载上
> 没有正收益路径。** `cava.pathfind.native` 保持默认 **false**（这正是那条门禁存在的意义）。
> 唯一还值得一试的方向（需要 captain 重新裁决 R1/R4）：**回退成本里有 42.9% 是纯浪费**（原生交回的结果
> 与 Java 逐节点相同），而"接管调用也慢"很可能与**安全修复引入的冷推送**（复用收紧为同 tick）有关 ——
> 新接上的失效钩子 + 金丝雀让"跨 tick 复用"第一次有了被验证的可能，那是唯一有机会把符号翻正的实验。
> 另一个方向是把 560 预算下"走不到目标"的长尾调用单独分流（那份长尾才是真正贵的），但这要先有推送成本的可控方案。


### 8.6.2 跨 tick 复用（R7）判决：**两重输**（`c527494` / `1d37037`，台账 `docs/CAVA-p1-cross-notes.md`）

**第一重：前提就不成立 —— 命中率 0.05%。** 复用要求"**同一个矩形**再次被求解"，而真实负载里每只怪/每个目标
算出来的矩形都不同。四条真实 AI 腿（40 僵尸 + 1 村民 × 2400 tick）合计 **1972 次调用 / 1972 次推送 / 跨 tick 复用命中 1 次**；
连**同 tick 复用都是 0**。干净判决数字（只算对照组）**1/1002 = 0.0998%**，判决线是 <50%。原文是
`mirrorPushes == nativeCalls`（499/503/497/473）—— **不是"几乎每次重推"，是每次都重推**。

**第二重：结构上就翻不正（这一条比命中率更重要）。** 把推送成本**整块抹掉**（任何"常驻镜像/增量更新"都到不了的
不可达上界）之后，四条腿仍然是 **−14.63 / −7.02 / −7.09 / −2.51 µs/tick（全 ≤ 0）**。原因：符号由
**回退白付**决定（8.5–16.7 µs/tick），而推送只占 8.4–11.0 µs/tick。
> ⇒ **我上一条给用户的解释（"亏在搬运"）只对了一半**：搬运确实要 28–30% 的原生耗时（39.89–52.97 µs/次），
> 但把它归零也救不回来。**真正的账是"回退 = 白付一次原生然后再跑一次 Java"**，而且回退恰好落在最贵的那些调用上。

**正确性那一半是做成了的（也顺手把跨 tick 失效真的验了）**：新增 `cava pathfind xtick arm/check`，
改窗口内方块后**推进真实 tick（2/6/21/101）**再求解 ⇒ 四条 `write` 臂全 PASS；
**关掉失效钩子的对照四条全 RED**（`no-invalidation`、两轮 `stale-result(B==A)` 原样交回改方块前的路径、
一轮 `B!=javaRef`；`nowrite` 臂更狠：复用真的命中却交回 **129** 节点陈旧路径 vs Java 参考 **128**）。
活 tick 下**世界自己每 tick 写 0.6–1.0 个窗口内方块** ⇒ 跨 tick 复用完全依赖失效源，没有"没人动地形"这种便宜。
**仍缺的失效源**（已登记）：区段卸载/重载（`SectionOriginRegistry` 按 `ChunkSection` **实例**建表 ⇒ 重载后是新实例、
**静默漏失效**）、复用路径不复查 `isReady`、绕过 `setBlockState` 的直写 ⇒ 最坏仍是**穿墙**。

**这一注剩下的唯一一条路 = R1**（让"窗口够不够"有 **sound** 信号，把回退率从 ~30% 压到 ≤5%）：
量级估计 **+7 ~ +15 µs/tick**，正好是能把 −11.5…−25.6 翻正的量级（旁证：两条实现逐节点一致率 62.9–71.8%）。
但必须记住**天花板**：真实负载里**寻路总耗时本身只有 34–45 µs/tick**（≈ tick 的 7–9%）——
即使把寻路整块变成免费，收益也就这么多，原生只能吃掉其中一部分。
"整块世界常驻镜像"**单独做不改变符号**（上界只有 8.4–11.0 µs/tick），要与 R1 叠加才有意义；
内存量级 484 区块 × 384 层 ≈ 4760 万格 ≈ **190 MB**（int32）。

**顺手落地 R5 的可观测性待办**：`RegionMirror.report()` 的计数现在 `/cava pathfind stats` 与 AIDIST 回执行都能看到
（含 `mirrorPushes / mirrorCrossTick / mirrorCrossBlocked / mirrorInval`）。
**默认值一个都没动**：`cava.mirror.reuse.crosstick=false`、`cava.pathfind.native=false`。
全量测试 265 tests / 0 failed / 9 skipped；交付物 DLL 未改动（`0ebe3b04…`）。
