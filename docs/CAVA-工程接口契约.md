...
10. **线程模型（P4-C 实测补充）：所有 `cava_*` 入口都由 Java 侧从同一个线程调用**
    （1.20.4 服务端里就是 Server thread；`cava_open`/`cava_close` 同此约束）。
    原生侧内部对**单个句柄的可变状态**（区段 / 状态表 / 形状表 / 档案）用一把全局互斥量保护，
    因此**不会出现数据损坏**（8 线程 × 4000 轮实测：无崩溃、无未定义返回码、无越界写、无撕裂读）；
    但它**不提供"调用序列级原子性"** —— 并发替换形状表期间，
    ...
    **调用方义务**：同一句柄上的所有原生调用必须由**同一条线程**发起，或由调用方**自己用一把锁串行化**
    （本机现状：`PathfindHook.nativeLock` 与 `RegionMirror` 的对象锁已经这么做了）。
    **违反的后果**：不会立刻崩，但会出现上面的 `CAVA_ERR_ARG` 抖动，且**任何并发正确性都不在本 ABI 的保证范围内**。
    **触发重开这条决策的条件**：引入任何**异步寻路/异步实体**的 mod（本整合包 49 个 mod 里没有），
    届时走候选方案 B（原生侧加锁/快照，估 3–8 人日）。
    （三选一的成本对比与实测数据：`docs/CAVA-concurrency-notes.md` §5。）
