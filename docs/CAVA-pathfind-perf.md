...
## 12. 【P1-NET 追加 2026-09-24】净收益终于有了数字：真实 AI 负载下**是净亏**

见 `docs/CAVA-p1-net-notes.md`（P1-NET 流）。三条要点（全部实跑，命令/回执/哈希在那边）：

1. **成对实测净收益 = −14.7 µs/tick**（同一次调用把 Java 与原生两条路都跑一遍：−35.405 / −35.309 ms ÷ 2400 tick）；
   机制：原生在**接管成功**的调用上就比 Java 慢 1.22x（157.7 vs 129.6 µs/次），另有 31–33% 的调用白付一次原生。
2. **按规模分流救不回来**：真实距离 p50=7/p90=10/max≈11–15 格、预算恒 560；逐距离桶**每一桶都是净亏**、
   回退率与规模无关（24.7/33.3/41.6/38.6%）⇒ 阈值只能砍前缀，砍哪一段都翻不正；
   数据支持的最优阈值是 ≥12 格 = 不接管。分流开关默认 **0（关）**。
3. **MSPT 分辨不出**（纯 Java 的三条腿 p50 就差 52%）⇒ 判据只能用成对计数。
   §8 那张"每 tick 投影"表（×0.22 次/tick）从此被**直测**取代。
   本节与 P1-FIX 的"回退率 47.8%/51.8%"不矛盾：本流的回退率是 21.1–37.5%（村民加了伤害免疫、窗口 2800 tick）。
