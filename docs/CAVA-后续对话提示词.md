...
| 阶段 | 结论 | 关键数字 |
| --- | --- | --- |
| P1 寻路 | **实现完成、正确性已验证，但判定不上线** —— `cava.pathfind.native` **默认 false**（保持不变） | 正确性：`-Compare` **CONSISTENT**（12/12 preset×mode 逐字段一致）、两条可证伪对照都变红；**但真实 AI 负载净收益 = −14.7 µs/tick（净亏），R2 三条判据全不满足 ⇒ NO-GO**。根因：真实 AI 每次调用距离 p50=7/p90=10 格、预算恒 560 ⇒ **这套整合包里没有大搜索空间**，而原生的跨界固定成本让"接管成功"的调用也比 Java 慢 1.22x |
...
- ★★ **P1 的两个必修缺陷 —— 已修复并双向验证（P1-FIX，`2790772`/`6365a0c`/`ddb919c`）**：
  `-Compare` → **CONSISTENT**（12/12 preset×mode 逐字段一致）、`invalidate` 5 个 preset PASS、
  两条"改坏必须变红"的对照都变红；但 **`cava.pathfind.native` 仍然默认关**（captain 裁决 R1–R6 见门禁 #8 §8.6.1）：
  真实 AI 回退率 **47.8%/51.8%**、短程搜索原生慢 11 倍 ⇒ 先要"真实负载净收益为正 + 回退率 ≤5% + 按规模分流"三条都满足。
  **P1-NET 流正在测那个净收益数字。**
- ★★ **P1 的最终账（P1-NET，`09ea1dc`）**：真实负载 **−14.75/−14.71 µs/tick**（成对口径，两次差 0.2%），
  7/7 条腿符号一致；**按规模分流实现并实测过，只能止亏不能转正**（数据支持的最优阈值 = 不接管）；
  亏损可精确分解：接管调用原生 157.7 µs vs Java 129.6 µs（1.22x，跨界固定成本）+ 31–33% 回退白付原生。
  ⇒ **在 R1 不变、ABI 冻结下 P1 没有正收益路径**；`cava.pathfind.native` 保持默认 **false**。
  ★ **跨 tick 复用那一注已经赌过了，判负（两重输，`c527494`/`1d37037`，见门禁 #8 §8.6.2）**：
  **①命中率 1/1972 = 0.05%** —— 真实负载里没有"第二个相同矩形"（`mirrorPushes == nativeCalls`，连同 tick 复用都是 0）；
  **②即使把推送成本整块抹零，四条腿仍 ≤ 0** ⇒ 符号由**回退白付**决定，不是搬运。我先前"亏在搬运"的解释只对了一半。
  **P1 剩下的唯一一条路是 R1**（给"窗口够不够"一个 sound 信号 ⇒ 回退率压到 ≤5% ⇒ 量级 +7~+15 µs/tick），
  但**天花板**是：真实负载里寻路总耗时只有 **34–45 µs/tick（tick 的 7–9%）**，即使寻路免费也就这么多。
  "整块世界常驻镜像"单独做不改变符号（上界 8.4–11.0 µs/tick，且要 ~190 MB 常驻副本），必须与 R1 叠加。
- **★ 第三个缺陷（教训比缺陷本身值钱）**：`Path.reachesTarget` 在 Java 侧被填反，而比对脚本把键名写成
  `reachesTargetFlag`（回执里是 `reachedTargetFlag`）⇒ **这个字段从来没被比过**，"逐字段一致"曾经是假的。
  ⇒ **"比对"本身也要机械对拍**（R6，交给 P1-NET 做）。
- 原始的两个缺陷（留档，说明当时是怎么被抓到的）：
